...
class ListsSuite extends FunSuite {
   ...
   */
  test("sum of a few numbers (1,2,3) = 3") {
    assert(sum(List(1, 2, 0)) === 3)
  }
  
  test("sum of an empty list = 0") {
    assert(sum(List()) === 0)
  }
  
  test("max of a few numbers (3, 7, 2) = 7") {
    assert(max(List(3, 7, 2)) === 7)
  }
  
  test("max of a few numbers (3, 7, 2, 7) = 7") {
    assert(max(List(3, 7, 2, 7)) === 7)
  }
  
  test("max of an empty list throws an NoSuchElementException"){
